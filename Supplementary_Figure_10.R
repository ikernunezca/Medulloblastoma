...
# Load multilayer data from Archer et al 2018. 
...
# For this case, we exclude patients with missing data on RNAseq. This 6 are: "x1M6","x5M15","Burdenko_1360","MB037","MB018","MB282",

ground_truth_patients <- c("MB244","MB268","MB274","MB275","MB102","MB104","MB234","MB239","MB284",
                           "MB136","MB266","MB287","MB088","MB206",
                           "MB095","MB106","MB170","MB226","MB247","MB248","MB260",
                           "MB277","MB164","MB271","MB166","MB278","MB288",
                           "MB177","MB199","MB281","MB174","MB227","MB265","MB270","MB264","MB099","MB091","MB118","MB269")
ground_truth <- matrix(nrow= length(ground_truth_patients),ncol=2)
ground_truth[,1] <- ground_truth_patients
ground_truth[,2] <- c(rep("SHHa",9),rep("SHHb",5),rep("G3a",7),rep("G3b",6),rep("G4",12))
...
for(u in 1:11){ # 1 to 11 because of non 0-based language: If we want, for example, theta to be 0, we set u=1
  for(val in 1:20){ #lambda
    ...
    WHATEVER <- c("x1M6","x5M15","Burdenko_1360","MB037","MB018","MB282") #Missing data patients
    ...
    res_hclust <- hclust(jaccard_ind(patient_matrix3),"ward.D2")
      ...
      if(u == 1){
        if(val==3){
          pdf("data/Plots/Supplementary_Figure_10.pdf",height= 14, width= 20)
...
write.table(final_accuracy_matrix,row.names=T,col.names=F,file="Supplementary_Tables/Supplementary_Table_7.tsv") # Supplementary 7
write.table(final_kk_used,row.names=T,col.names=F,file="Supplementary_Tables/Supplementary_Table_8.tsv") # Supplementary 8
write.table(final_matthews_matrix,row.names=T,col.names=F,file="Supplementary_Tables/Supplementary_Table_9.tsv") # Supplementary 9
